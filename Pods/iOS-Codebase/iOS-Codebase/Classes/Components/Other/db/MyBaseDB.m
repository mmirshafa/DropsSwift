...
- (MyDBResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
@end

@implementation MyBaseDB
...
- (void)clearCachedStatements {
    ...
    for (DFStatement *cachedStmt in [_cachedStatements objectEnumerator]) {
        [cachedStmt close];
    }
...
//    NSMutableSet *openSetCopy = FMDBReturnAutoreleased([_openResultSets copy]);
    NSMutableSet *openSetCopy = [_openResultSets copy];
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        MyDBResultSet *rs = (MyDBResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(MyDBResultSet *)resultSet {
    NSValue *setValue = [NSValue valueWithNonretainedObject:resultSet];
    
    [_openResultSets removeObject:setValue];
}

- (DFStatement*)cachedStatementForQuery:(NSString*)query {
    return [_cachedStatements objectForKey:query];
}

- (void)setCachedStatement:(DFStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    MyDBResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (MyDBResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    DFStatement *statement  = 0x00;
    MyDBResultSet *rs         = 0x00;
...
//    FMDBRetain(statement); // to balance the release below
    ...
    if (!statement) {
        statement = [[DFStatement alloc] init];
    ...
    rs = [MyDBResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
//            FMDBRelease(parameterName);
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[DFStatement alloc] init];
