...
+ (id)resultSetWithStatement:(DFStatement *)statement usingParentDatabase:(MyBaseDB*)aDB {
    ...
    MyDBResultSet *rs = [[MyDBResultSet alloc] init];
...
- (unsigned long)unsignedLongForColumn:(NSString*)columnName {
    return [self unsignedLongForColumnIndex:[self columnIndexForName:columnName]];
}

- (unsigned long)unsignedLongForColumnIndex:(int)columnIdx {
    return (unsigned long)[self longLongIntForColumnIndex:columnIdx];
}
...
- (id)objectForColumnIndex:(int)columnIdx {
    ...
    if (returnValue == nil && _returnNSNullInsteadOfNaN) {
